...
>  1.初始化8259中断控制器，配置好相应的参数   
>  2.建立好中断描述符IDT，建立相应的中断号和段基址   
>  3.建立好全局描述符GDT   
>  4.初始化8253这个和时钟外设相关的芯片    
>  5.使能中断
...
3:	1 2 3 
5:	1 2 3 4 5 
8:	1 2 3 4 5 0 2031599301 0 
```

如果让你来一个阶段一个阶段地从零开始完整实现lab1（不是现在的填空考方式），你的实现步骤是什么？（比如先实现一个可显示字符串的bootloader（描述一下要实现的关键步骤和需要注意的事项），再实现一个可加载ELF格式文件的bootloader（再描述一下进一步要实现的关键步骤和需要注意的事项）...） (spoc)
- [x]  

> 可以显示字符串的bootloader：设置启动地址、设置中断使能为不可中断、初始化相应的寄存器、初始化GDT、等待外设空闲、输出到外设上。   
可以加载elf格式文件的bootloader：设置elf文件格式的一系列系数，判断是否为elf文件，加载相应的扇区Kernel。


如何能获取一个系统调用的调用次数信息？如何可以获取所有系统调用的调用次数信息？请简要说明可能的思路。(spoc)
- [x]  

> 1. strace常用来跟踪进程执行时的系统调用和所接收的信号。在Linux中，进程不能直接访问硬件设备，当进程需要访问硬件设备(比如读取磁盘文件，接收网络数据等等)时，必须由用户态模式切换至内核态模式，通过系统调用访问硬件设备。strace可以跟踪到一个进程产生的系统调用,包括参数，返回值，执行消耗的时间。   
strace的不同参数对应着不同的功能，比如-c表示统计每一系统调用的所执行的时间,次数和出错的次数等。   
2.或者在程序中建立两个表（数组）A与B，A用于存放每次调用的具体信息（如时间、维持时长等），B用于表示对应的A的系统调用时系统调用的次数，每次系统调用时，先将B之前的调用次数加一，存入下一个地址，然后再将系统调用的具体信息写入A的下一个地址，这样就可以实现查询系统调用的次数等信息了。
